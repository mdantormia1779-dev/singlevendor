...
const OrderSummery = ({ items = [] }) => {
  ...
  const subtotal = items.reduce((acc, item) => acc + parseInt(item.price.replace(/,/g, '')), 0);
      ...
      <Card className="rounded-2xl border border-gray-100 shadow-sm">
        <CardContent className="p-6">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between text-gray-600">
              <span>Savings</span>
              <span className="font-medium text-red-600">-৳{savings.toLocaleString()}</span>
            </div>
          </div>
          ...
          <div className="flex justify-center gap-6 mt-4 text-xs text-gray-400">
            <div className="flex items-center gap-1"><ShieldCheck size={14} /> Secure</div>
