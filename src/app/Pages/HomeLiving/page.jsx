...
const HOME_KEYWORDS = ["Home", "Living", "Decor", "Lamp", "Kitchen", "Bed", "Sofa", "Chair", "Table", "Room"];
...
export default function HomeLivingPage() {
  ...
  useEffect(() => {
    fetch("/api/products")
      ...
      .catch((err) => console.error("Home & Living products fetch error:", err))
  ...
  const categories = useMemo(() => {
    const counts = {};
    baseList.forEach((p) => {
      const cat = p.category || "Home & Living";
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              {loading ? "Loading catalog..." : `${sortedProducts.length} items found`}
            </p>
          </div>
